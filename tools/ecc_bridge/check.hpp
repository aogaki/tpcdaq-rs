...
//
// **出自**: tools/root_sink/check.hpp の逐語コピー(+ 末尾に CHECK_STR を追加)。
// ecc-bridge は root-sink と別所有・別ビルドなので、ディレクトリを跨いで include
// せずコピーで独立させている(TODO/017「tools/ecc_bridge/ 内で完結」)。
//
// フレームワークを持ち込まない。CHECK は失敗しても走り続け、最後に件数を出して
// 非 0 で終わる(どのケースが落ちたか全部見える方が原因究明が速い)。

#ifndef TPCDAQ_ECC_BRIDGE_CHECK_HPP
#define TPCDAQ_ECC_BRIDGE_CHECK_HPP
...
#endif  // TPCDAQ_ECC_BRIDGE_CHECK_HPP
