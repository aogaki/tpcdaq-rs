...
- **ZS 有効化の意向**(§7。閾値の物理と、ZS 済みデータでのオフラインペデスタル戦略)
...
| ゲイン正規化(strip 毎) | ○(greenfield) | TPCReco に**無い**。FW 側に AsAd pulser 較正ランプ + configure-pulser.xcfg あり | 較正 run 手順(既存 run 制御で可)+ ゲイン表生成ツール + 適用(ROOT プロダクト/表示) | 較正の頻度・手順の合意 |
